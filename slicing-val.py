...
def apply_laplacian_slicing(mat):
    neigh_mat = -4 * mat
...
def update(A, B, DA, DB, f, k, delta_t, laplacian_fn):
    """Apply the Gray-Scott update formula"""

    # compute the diffusion part of the update
    diff_A = DA * laplacian_fn(A)
    diff_B = DB * laplacian_fn(B)
...
# original
np.random.seed(42)
A0, B0 = get_initial_A_and_B(N)
for step in range(N_steps):
    A0, B0 = update(A0, B0, DA, DB, f, k, delta_t, apply_laplacian)

# new laplcains
np.random.seed(42)
A1, B1 = get_initial_A_and_B(N)
for step in range(N_steps):
    A1, B1 = update(A1, B1, DA, DB, f, k, delta_t, apply_laplacian_slicing)


print(f"A match: {np.allclose(A0, A1, atol=1e-10)}")
print(f"B match: {np.allclose(B0, B1, atol=1e-10)}")
print(f"A max diff: {np.max(np.abs(A0 - A1)):.2e}")
print(f"B max diff: {np.max(np.abs(B0 - B1)):.2e}")